...
// --- 2. GÉOGRAPHIE --- (dérivé automatiquement depuis COUNTRY_COORDINATES — source unique de vérité)
// NB : défini plus bas, après COUNTRY_COORDINATES, pour éviter une référence avant déclaration.
...
// --- VOYAGES (Storyboards) ---
// Un voyage regroupe plusieurs observations sous une même histoire
// (ex: "Mon voyage aux Philippines" du 12/03 au 25/03)
export interface Trip {
  ...
  color: string; // Couleur d'accent du voyage
  notes?: string;
...
export const TRIP_COLORS = [
  "#006994", // Bleu marine principal
  "#0288D1", // Bleu lagon
  "#26A69A", // Turquoise
  "#00838F", // Bleu canard
  "#F57C00", // Corail / coucher de soleil
  "#7E57C2", // Violet abyssal
  "#43A047", // Vert algue
  "#EC407A", // Rose poisson clown
];
...
// --- 2. GÉOGRAPHIE --- dérivé de COUNTRY_COORDINATES (source unique de vérité)
// Tous les pays et leurs océans sont automatiquement synchronisés.
export const GEOGRAPHY_DB: Record<string, string[]> = Object.fromEntries(
  Object.entries(COUNTRY_COORDINATES).map(([country, oceans]) => [
    country,
    Object.keys(oceans),
  ]),
);
